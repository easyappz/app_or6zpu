...
import { getAd, updateAd } from '../api/ads.js';
...
function AdEdit() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setServerError(typeof detail === 'string' ? detail : 'Не удалось загрузить объявление');
      } finally {
  ...
  async function onSubmit(e) {
    ...
    try {
      setSubmitting(true);
      const updated = await updateAd(id, payload);
      ...
      if (data && typeof data === 'object') {
        ...
        setServerError('Ошибка при сохранении объявления');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <div data-easytag="id2-src/pages/AdEdit.jsx" style={{ maxWidth: 780, margin: '0 auto' }}>
        Загрузка…
      </div>
    );
  }
  ...
  return (
    <div data-easytag="id2-src/pages/AdEdit.jsx" style={{ maxWidth: 780, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, fontWeight: 600, marginBottom: 16 }}>Редактирование объявления</h1>
      {serverError && (
        <div style={{ background: '#fff1f0', border: '1px solid #ffa39e', color: '#a8071a', padding: '12px 16px', borderRadius: 8, marginBottom: 16 }}>
          {serverError}
        </div>
      )}
      <form data-easytag="id2-form-src/pages/AdEdit.jsx" onSubmit={onSubmit} style={{ display: 'grid', gap: 16 }}>
        <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div>
            ...
            <select id="category" value={form.category_id} onChange={(ev) => updateField('category_id', ev.target.value)} style={inputStyle}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
          <button type="submit" disabled={submitting} style={primaryButtonStyle}>{submitting ? 'Сохранение…' : 'Сохранить изменения'}</button>
          <Link to={`/ads/${id}`} style={linkButtonStyle}>Отмена</Link>
        </div>
      </form>
    </div>
  );
